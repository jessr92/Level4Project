...
Summary: Algorithm and device dependent changes to local size (number of work
items in each dimension in a work group) affect performance. Large (128) =
better latency hiding on GPU. Phi likes local size of 4 (4-way ht likely
reason). If local size too big, compute unit used inefficiently (say compute
unit can take 511 work items simultaneously (based on private memory usage), and
local size is 256. Only 1 work group (256 work items) can be enqueued. If local
size is 4, 127 work groups (508) work items can be enqueued). global size mod
local size has to be 0.

Graph of local size effect on Tesla Figure~\ref{fig:teslaLocalSize}
...
Graph of local size effect on Intel Xeon Phi Figure~\ref{fig:phiLocalSize}
